document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadStorageData() {
    if (localStorage.getItem("utm_param_data")) {
      let storedData = JSON.parse(
        localStorage.getItem("utm_param_data") as string
      );
  ...
  function searchUTMParams() {
    ...
    paramsList.forEach((param) => {
      if (urlParams.get(param)) {
        detectedParams[param] = urlParams.get(param) as string;
      }
    });
  }
  ...
  function setData() {
    ...
    if (localStorage.getItem("utm_param_data")) {
      expiry = new Date(
        JSON.parse(localStorage.getItem("utm_param_data") as string).expiry
      );
    }
  ...
  function setParamsToLinks() {
    let elms = document.querySelectorAll(abhinash_utm_plugin_data.selector);
